...
export function postingTdsPercent(transactionType${optStr})${typed ? ': number | null' : ''} {
  const found = getPostingRule(transactionType);
  if (!found) return null;
...
export function postingTdsAccount(transactionType${optStr})${retStrNull} {
...
export function postingDiscountAccount(transactionType${optStr})${retStrNull} {
  return getPostingRule(transactionType)?.discountAccount || null;
}

/** True when DISCOUNT JVS is on and Disc. A/C is filled. */
export function postsDiscountJournal(transactionType${optStr})${retBool} {
  const found = getPostingRule(transactionType);
 ...
 */
export function resolveDiscountJournal(transactionType${optStr}, discountAmount${typed ? '?: number | null' : ''})${typed ? ': { amount: number; account: string } | null' : ''} {
 ...
 */
export function resolveDefaultTdsPercent(transactionType${optStr}, suggestedFromPan${typed ? '?: number | null' : ''})${typed ? ': number | null' : ''} {
