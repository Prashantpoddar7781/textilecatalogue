/**
 * Credit / debit notes from Empire TRANSACTION TYPES.
 * Series used: CREDIT NOTE (ON SALES) P91, CREDIT NOTE (ON PURCHASES) P92,
 * DEBIT NOTE (ON SALES) S91, DEBIT NOTE (ON PURCHASES) S92.
 * TDS/TCS-only series stay out of this pick list.
 */
...
export const CREDIT_DEBIT_NOTE_TYPES: CreditDebitNoteType[] = [
  {
    value: 'CREDIT NOTE (ON SALES)',
    series: 'CREDIT NOTE (ON SALES)',
    seriesCode: 'P91',
    label: 'CREDIT NOTE (ON SALES)',
    noteKind: 'credit',
    noteSide: 'sales',
    partyType: 'customer'
  },
  {
    value: 'CREDIT NOTE (ON PURCHASES)',
    series: 'CREDIT NOTE (ON PURCHASES)',
    seriesCode: 'P92',
    label: 'CREDIT NOTE (ON PURCHASES)',
    noteKind: 'credit',
    noteSide: 'purchase',
    partyType: 'supplier'
  },
  {
    value: 'DEBIT NOTE (ON SALES)',
    series: 'DEBIT NOTE (ON SALES)',
    seriesCode: 'S91',
    label: 'DEBIT NOTE (ON SALES)',
    noteKind: 'debit',
    noteSide: 'sales',
    partyType: 'customer'
  },
  {
    value: 'DEBIT NOTE (ON PURCHASES)',
    series: 'DEBIT NOTE (ON PURCHASES)',
    seriesCode: 'S92',
    label: 'DEBIT NOTE (ON PURCHASES)',
    noteKind: 'debit',
    noteSide: 'purchase',
    partyType: 'supplier'
  }
...
export function parseNoteType(value?: string | null): CreditDebitNoteType | undefined {
  ...
  const exact = CREDIT_DEBIT_NOTE_TYPES.find(
    type => type.value === aliased || type.series === aliased || type.seriesCode === aliased.toUpperCase()
  );
...
export function parseNoteTypeFromPath(pathSegment: string): CreditDebitNoteType | undefined {
  return parseNoteType(pathSegment);
}

export function noteTypeFromKindSide(noteKind?: string | null, noteSide?: string | null): CreditDebitNoteType {
  return parseNoteType(`${noteKind || ''} note (${noteSide || ''})`) || CREDIT_DEBIT_NOTE_TYPES[0];
}

export function formatNoteNumber(series: string, voucherNumber: number | string | null | undefined): string {
