...
const thClass = 'border border-fuchsia-200 bg-fuchsia-50 px-2 py-2 text-center text-[10px] font-black uppercase tracking-wide text-fuchsia-950';
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {isDetailed ? (
              <label className="block">
                ...
                <input className={inputClass} value={mainScreen} onChange={e => setMainScreen(e.target.value)} />
              </label>
            ) : (
            ...
            <div className="flex items-end">
              <button type="button" onClick={() => void load()} className="w-full rounded-xl bg-fuchsia-600 px-4 py-2.5 text-xs font-black uppercase text-white">
                Show Report
              </button>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            {isDetailed ? (
              <table className="w-full min-w-[1100px] border-collapse text-xs" style={{ tableLayout: 'fixed' }}>
                    ...
                    <React.Fragment key={group}>
                      {showGroup && (
                        <tr className="bg-fuchsia-100">
                          <td colSpan={detailedCols} className={`${tdClass} border-fuchsia-200 bg-fuchsia-100 text-sm font-black text-fuchsia-950`}>
                            {group}
                          </td>
                        </tr>
                      )}
                      {groupRows.map(row => (
                        <tr
                          key={row.id}
                          onClick={() => { window.location.href = `/erp/sales?edit=${row.billId}&kind=bill`; }}
                        ...
                        >
                          ...
                          {showTypeCol && (
                            <td className={`${tdCenter} text-[10px] font-black uppercase ${String(row.transactionType || '').includes('RETURN') ? 'text-amber-800' : 'text-fuchsia-800'}`}>
                ...
                <tfoot>
                  <tr className="bg-fuchsia-900 text-white">
                    <td colSpan={showTypeCol ? 6 : 5} className={`${tdClass} border-fuchsia-800 text-right font-black`}>Grand Total</td>
                    <td className={`${tdNum} border-fuchsia-800 font-black`}>{money(totals.pcs)}</td>
                    <td className={`${tdClass} border-fuchsia-800`} />
                    <td className={`${tdClass} border-fuchsia-800`} />
                    <td className={`${tdNum} border-fuchsia-800 font-black`}>{money(totals.grossAmount)}</td>
                    <td className={`${tdClass} border-fuchsia-800`} />
                    <td className={`${tdClass} border-fuchsia-800`} />
                  </tr>
                </tfoot>
              ...
              <table className="w-full min-w-[1280px] border-collapse text-xs" style={{ tableLayout: 'fixed' }}>
                ...
                <tbody>
                  ...
                  {!loading && grouped.map(([group, groupRows]) => (
                    <React.Fragment key={group}>
                      {showGroup && (
                        <tr className="bg-fuchsia-100">
                          <td colSpan={registerCols} className={`${tdClass} border-fuchsia-200 bg-fuchsia-100 text-sm font-black text-fuchsia-950`}>
                            {group}
                          </td>
                        </tr>
                      )}
                      {groupRows.map(row => (
                        <tr
                          key={row.id}
                          onClick={() => { window.location.href = `/erp/sales?edit=${row.billId || row.id}&kind=bill`; }}
                          className="cursor-pointer hover:bg-fuchsia-50"
                        >
                          <td className={`${tdClass} font-bold`}>{row.partyName}</td>
                          <td className={tdCenter}>{formatDate(row.date)}</td>
                ...
                <tfoot>
                  <tr className="bg-fuchsia-900 text-white">
                    <td colSpan={showTypeCol ? 8 : 7} className={`${tdClass} border-fuchsia-800 text-right font-black`}>Grand Total</td>
                    {['pcs', 'mts', 'grossAmount', 'taxableAmount', 'ledgerAmount', 'invoiceValue'].map(key => (
                      <td key={key} className={`${tdNum} border-fuchsia-800 font-black`}>{money(totals[key])}</td>
                    ))}
                    <td className={`${tdClass} border-fuchsia-800`} />
                  </tr>
                </tfoot>
