...
const VIEW_OPTIONS = [
  ...
  { value: 'detailed', label: 'Finish Sales Detailed Report' },
...
export const FinishSalesReportPage: React.FC<Props> = ({ onBack, erpSession }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err.message || 'Could not load Finish Sales report.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F6F7FB]">
      <ErpTopMenu title="Finish Sales Report" erpSession={erpSession} showSessionActions onBackToCatalogue={onBack} />
      <main className="mx-auto max-w-[1400px] px-4 py-6">
        ...
        <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
          <div className="border-b bg-fuchsia-50 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-fuchsia-800">
            {VIEW_OPTIONS.find(option => option.value === view)?.label || 'Finish Sales Report'} · Click any row to edit
          </div>
            ...
            {isDetailed ? (
              <table className="min-w-[1300px] w-full text-left text-xs">
                <thead className="border-b bg-gray-50 text-[10px] uppercase text-gray-500">
                  <tr>
                    {['Customer', 'Date', 'Bill No.', 'Main Screen', 'Screen Name', 'PCS', 'Cut', 'Rate', 'Gross Amt.', 'Haste', 'Broker'].map(head => (
                      <th key={head} className="px-2 py-2 font-black">{head}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {loading && <tr><td colSpan={11} className="p-10 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-fuchsia-600" /></td></tr>}
                  {!loading && rows.length === 0 && <tr><td colSpan={11} className="p-10 text-center font-bold text-gray-400">No Finish Sales bills found.</td></tr>}
                  {!loading && grouped.map(([group, groupRows]) => (
                    <React.Fragment key={group}>
                      {(view === 'party' || view === 'date' || view === 'broker' || view === 'station') && (
                        <tr className="bg-fuchsia-50/70"><td colSpan={11} className="px-2 py-2 text-sm font-black text-fuchsia-900">{group}</td></tr>
                      )}
                      {groupRows.map(row => (
              ...
              <table className="min-w-[1450px] w-full text-left text-xs">
                ...
                <tbody>
                  {loading && <tr><td colSpan={14} className="p-10 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-fuchsia-600" /></td></tr>}
                  {!loading && rows.length === 0 && <tr><td colSpan={14} className="p-10 text-center font-bold text-gray-400">No Finish Sales bills found.</td></tr>}
                  {!loading && grouped.map(([group, groupRows]) => (
                    <React.Fragment key={group}>
                      {(view === 'party' || view === 'date' || view === 'broker' || view === 'station') && (
                        <tr className="bg-fuchsia-50/70"><td colSpan={14} className="px-2 py-2 text-sm font-black text-fuchsia-900">{group}</td></tr>
                      )}
                      {groupRows.map(row => (
                ...
                <tfoot className="bg-fuchsia-900 font-black text-white">
                  <tr>
                    <td colSpan={7} className="px-2 py-2 text-right">Grand Total</td>
