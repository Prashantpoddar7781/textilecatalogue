...
export const MillReceiptTakaModal: React.FC<Props> = ({ open, rows, onClose, onApply, mode = 'FINISH' }) => {
  ...
  const selectedRows = useMemo(() => {
    return rows
      ...
      .map(row => {
        ...
        const recMts = isReturn ? greyMts : round2(Number(recMtsBySr[row.srNo]) || 0);
  ...
  }, [rows, selected, recMtsBySr, isReturn]);
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4">
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b px-4 py-3">
          <div>
            <h3 className="text-sm font-black uppercase tracking-wide text-gray-900">
              {isReturn ? 'Select Returned Takas' : 'Taka Details'}
            </h3>
            <p className="text-xs text-gray-500">
              Grey Mts: {totals.greyMts.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
              {' · '}
              {isReturn ? 'Return Mts' : 'Fin Mts'}: {totals.recMts.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {rows.length === 0 ? (
            <p className="py-8 text-center text-sm text-gray-500">
              {isReturn ? 'No taka left to return for this dispatch.' : 'No taka left to receive for this dispatch.'}
            </p>
          ) : (
            <table className="min-w-full text-left text-sm">
              <thead className="border-b bg-gray-50 text-[10px] uppercase text-gray-500">
                <tr>
                  ...
                  <th className="p-2 text-right">{isReturn ? 'Return Mts' : 'Rec Mts'}</th>
        ...
        <div className={`border-t px-4 py-3 ${isReturn ? 'bg-amber-50' : 'bg-teal-50'}`}>
          <div className="flex flex-wrap gap-4 text-sm">
            <p>
              <span className="font-black text-gray-600">{isReturn ? 'Return Taka:' : 'Rec. Taka:'}</span>{' '}
              <span className={`font-black ${isReturn ? 'text-amber-900' : 'text-teal-900'}`}>{totals.taka}</span>
            </p>
            <p>
              <span className="font-black text-gray-600">Grey Mts:</span>{' '}
              <span className={`font-black ${isReturn ? 'text-amber-900' : 'text-teal-900'}`}>
                {totals.greyMts.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
              </span>
            </p>
            <p>
              <span className="font-black text-gray-600">{isReturn ? 'Return Mts:' : 'Fin Mts:'}</span>{' '}
              <span className={`font-black ${isReturn ? 'text-amber-900' : 'text-teal-900'}`}>
                {totals.recMts.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
              </span>
            </p>
          </div>
          <div className="mt-3 flex justify-end gap-2">
            ...
            <button
              ...
              className={`rounded-xl px-4 py-2 text-sm font-black text-white disabled:opacity-60 ${
                isReturn ? 'bg-amber-700' : 'bg-teal-700'
              }`}
            >
              {isReturn ? 'Apply Returned Takas' : 'Apply Received Takas'}
            </button>
          </div>
        </div>
      </div>
