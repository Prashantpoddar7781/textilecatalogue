...
export const WorkReceiptReportPage: React.FC<Props> = ({ onBack, erpSession }) => {
  ...
  const [totals, setTotals] = useState({ recPcs: 0, recMts: 0, taxableAmount: 0, invoiceValue: 0 });
  ...
  const load = async () => {
    ...
    try {
      ...
      setRows(result.rows || []);
      setTotals(result.totals || { recPcs: 0, recMts: 0, taxableAmount: 0, invoiceValue: 0 });
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-[#F6F7FB]">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        ...
        <div className="mb-4 rounded-2xl border bg-white p-4 shadow-sm">
          <h1 className="text-lg font-black uppercase text-gray-900">Work Receipt Report</h1>
          <p className="text-xs font-semibold text-fuchsia-800">Company: {companyName || '-'}</p>
            ...
            <table className="min-w-full text-left text-xs">
              <thead className="border-b bg-fuchsia-50 text-[10px] uppercase text-fuchsia-900">
                <tr>
                  ...
                  <th className="px-3 py-2">Desp Chal.</th>
                  <th className="px-3 py-2">Item Name</th>
                  <th className="px-3 py-2">Job Type</th>
                  <th className="px-3 py-2 text-right">Rec Pcs</th>
              ...
              <tbody>
                {rows.map(row => (
                  <tr key={row.id} className="border-b hover:bg-fuchsia-50/50">
                    <td className="px-3 py-2 font-semibold">{row.partyName}</td>
                    <td className="px-3 py-2">{formatDate(row.date)}</td>
                    <td className="px-3 py-2 font-bold text-fuchsia-800">{row.challanNo}</td>
                    ...
                    <td className="px-3 py-2 text-right font-bold">{money(row.amount ?? row.taxableAmount)}</td>
                  </tr>
                ))}
                {!rows.length && (
                  <tr><td colSpan={9} className="px-3 py-10 text-center text-sm text-gray-500">No work receipt entries.</td></tr>
                )}
              </tbody>
              <tfoot>
                <tr className="bg-slate-900 text-[11px] font-black uppercase text-white">
                  <td className="px-3 py-2" colSpan={6}>Grand Total · Invoice {money(totals.invoiceValue)}</td>
                  <td className="px-3 py-2 text-right">{totals.recPcs}</td>
                  <td className="px-3 py-2 text-right">{money(totals.recMts)}</td>
                  <td className="px-3 py-2 text-right">{money(totals.taxableAmount)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
